...
class NpmBinWrapperTests(unittest.TestCase):
    def test_node_wrapper_invokes_cli_help(self) -> None:
        ...
        self.assertIn(
            "{home,about,auth,whoami,ideas,idea,feature,world,secrets,link,runner,checkpoint,agent,discover,collaborate,init,status,branch,backup,ready,doctor,cleanup,gate,packet,erdos,pack,report}",
            proc.stdout,
        )
