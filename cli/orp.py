...
def _default_youtube_note_path(repo_root: Path, video_id: str) -> Path:
    return repo_root / "analysis" / "youtube" / f"{video_id}.md"


def _youtube_source_note_markdown(
    payload: dict[str, Any],
    repo_root: Path,
    *,
    source_artifact_path: str = "",
) -> str:
    title = str(payload.get("title", "")).strip() or "YouTube Source Note"
    canonical_url = str(payload.get("canonical_url", "")).strip()
    video_id = str(payload.get("video_id", "")).strip()
    author_name = str(payload.get("author_name", "")).strip()
    duration_seconds = payload.get("duration_seconds")
    published_at = str(payload.get("published_at", "")).strip()
    transcript_available = bool(payload.get("transcript_available", False))
    transcript_language = str(payload.get("transcript_language", "")).strip()
    transcript_track_name = str(payload.get("transcript_track_name", "")).strip()
    transcript_track_source = str(payload.get("transcript_track_source", "")).strip()
    transcript_fetch_mode = str(payload.get("transcript_fetch_mode", "")).strip()
    transcript_track_count = payload.get("transcript_track_count") or 0
    warnings = payload.get("warnings") if isinstance(payload.get("warnings"), list) else []
    warning_text = ", ".join(str(item).strip() for item in warnings if str(item).strip()) or "none"
    source_artifact_line = (
        f"- Source artifact: `{source_artifact_path}`"
        if source_artifact_path
        else "- Source artifact: not saved locally yet; rerun with `--save` to persist the structured JSON artifact"
    )

    lines = [
        f"# {title}",
        "",
        f"Source URL: {canonical_url or '(unknown)'}  ",
        f"Video ID: `{video_id or '(unknown)'}`",
        "",
        "## Source Metadata",
        "",
        f"- Title: `{title}`",
        f"- Channel: `{author_name or '(unknown)'}`",
        f"- Duration seconds: `{duration_seconds if duration_seconds not in (None, '') else '(unknown)'}`",
        f"- Published: `{published_at or '(unknown)'}`",
        source_artifact_line,
        "",
        "## ORP Transcript Retrieval",
        "",
        f"- Transcript available: `{str(transcript_available).lower()}`",
        f"- Language: `{transcript_language or '(unknown)'}`",
        f"- Track name: `{transcript_track_name or '(unknown)'}`",
        f"- Track source: `{transcript_track_source or '(unknown)'}`",
        f"- Fetch mode: `{transcript_fetch_mode or '(unknown)'}`",
        f"- Track count found: `{transcript_track_count}`",
        f"- Warnings: {warning_text}",
        "",
        "This note is an ORP-native working interpretation scaffold generated from `orp youtube inspect`.",
        "It is for synthesis, momentum, and downstream artifact creation. It is not canonical evidence by itself.",
        "",
        "## What This Says",
        "",
        "_Write the clearest 1-3 paragraph summary of what the source is actually saying._",
        "",
        "## Extracted Claims",
        "",
        "- _Claim 1_",
        ...
        "",
        "_What energy, direction, confidence, or strategic clarity does this source give the project?_",
        "",
        "## How This Helps Us Right Now",
        "",
        "_Explain how this source helps the current repo, product direction, architecture, or research posture._",
        "",
        "## Relationship To The Current Project",
        "",
        "- _Kernel relevance_",
        "- _Runner / execution relevance_",
        "- _Source-ingestion relevance_",
        "- _CLI / protocol relevance_",
        "",
        "## Knowledge Synthesis",
        "",
        "_Synthesize the source into a sharper project-level insight, not just a recap._",
        "",
        "## Possible Action Paths",
        "",
        "_Optional. Capture implied next moves for agent context only. Do not execute them automatically unless explicitly requested._",
        "",
        "- _Possible path 1_",
        "- _Possible path 2_",
        "",
        "## Candidate ORP Artifact Conversions",
        "",
        "- `decision`: _if this source should sharpen a project decision_",
        "- `hypothesis`: _if this source suggests something testable_",
        "- `policy`: _if this source implies a durable operating rule_",
        "- `checkpoint`: _if this source updates current project understanding_",
        "",
        "## Short Excerpts",
        "",
        "_Optional. Keep excerpts brief._",
    ]
    return "\n".join(lines).rstrip() + "\n"
...
def cmd_youtube_inspect(args: argparse.Namespace) -> int:
    ...
    emitted_format = ""
    note_out_raw = str(getattr(args, "note_out", "") or "").strip()
    should_save_note = bool(getattr(args, "save_note", False) or note_out_raw)
    ...
    if should_save_note:
        if note_out_raw:
            note_path = _resolve_cli_path(note_out_raw, repo_root)
        else:
            note_path = _default_youtube_note_path(repo_root, str(payload.get("video_id", "")).strip())
        if note_path.exists() and not bool(getattr(args, "force", False)):
            raise RuntimeError(
                f"note path already exists: {_path_for_state(note_path, repo_root)}. Use --force to overwrite."
            )
    if out_path is not None and note_path is not None and out_path == note_path:
        raise RuntimeError("source artifact path and note path must be different.")
    if should_save:
        emitted_format = _write_structured_payload(out_path, payload, format_hint=str(getattr(args, "format", "") or ""))
    if note_path is not None:
        source_artifact_ref = _path_for_state(out_path, repo_root) if out_path is not None else ""
        note_text = _youtube_source_note_markdown(payload, repo_root, source_artifact_path=source_artifact_ref)
    ...
    result = {
        ...
        "note_saved": note_path is not None,
        "note_path": _path_for_state(note_path, repo_root) if note_path is not None else "",
    ...
    if args.json_output:
        ...
        _print_pairs(
            [
                ...
                ("path", _path_for_state(out_path, repo_root) if out_path is not None else ""),
                ("note.saved", str(bool(note_path is not None)).lower()),
                ("note.path", _path_for_state(note_path, repo_root) if note_path is not None else ""),
            ]
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s_youtube_inspect.add_argument(
        "--save-note",
        action="store_true",
        help="Save an ORP-native Markdown source note scaffold under analysis/youtube/<video_id>.md",
    )
    s_youtube_inspect.add_argument(
        "--out",
        default="",
    ...
    s_youtube_inspect.add_argument(
        "--note-out",
        default="",
        help="Optional output path for the Markdown source note (.md)",
    )
    s_youtube_inspect.add_argument(
        "--format",
        default="",
